...
- **🧪 测试工具**：内置测试模板创建命令，便于测试各种变量类型功能
- **🔄 实时同步**：文件监听和自动刷新，多窗口间数据实时同步
...
扩展支持在提示词中使用变量，让提示词更加灵活和可重用。

### 变量语法

在提示词内容中使用 `{{变量名}}` 语法定义变量位置：
...
1. **文本输入** (`text`) - 单行文本输入
2. **多行文本** (`multiline`) - 多行文本输入
3. **选择框** (`select`) - 从预定义选项中选择
4. **数字** (`number`) - 数字输入
5. **日期** (`date`) - 日期选择器

### 变量定义

在提示词的 `variables` 数组中定义变量：
...
- ✅ **优雅降级**：提供编辑器插入作为备选方案
- ✅ **测试模板**：内置测试模板创建命令，包含所有变量类型用于功能测试
- ✅ **实时刷新**：文件监听和事件系统，确保多窗口间数据实时同步
